...
We can also use the stepAIC function - it will produce the same result as using 
the step function (as the step function bases model selection on AIC rather than 
F-ratio improvement). Step is actually a slightly simplified version of the 
stepAIC function.
...
In this case, the model with the lowest AIC value is also the one arrived at 
statisically via the sequential procedure - but this may not always be the case.
